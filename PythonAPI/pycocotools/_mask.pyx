...
cdef class RLEs:
    cdef RLE *_R
    cdef siz _n
    #cdef int[::1] cnts = <int[:self._n]> self._R[i].cnts
    ...
    def __getitem__(self, item):
        ...
        shape[0] = <np.npy_intp> self._R[item].m
        # Create a 1D array
        ndarray = np.PyArray_SimpleNewFromData(1, shape, np.NPY_UINT32, self._R[item].cnts)
...
cdef int binarySearch(int arr[], int low, 
    ...
    cdef int mid = (low + high)/2; # /*low + (high - low)/2;*/
    if (key == arr[mid]):
        return mid;
    elif (key > arr[mid]):
...
def _toString(RLEs Rs, int binary):
    cdef siz n = Rs.n
    ...
    for i in range(n):
        ...
        objs.append({
            ...
            'counts': py_string if binary else py_string.decode('ascii')
        })
        free(c_string)
    return objs
...
def encode(np.ndarray[np.uint8_t, ndim=3, mode='fortran'] mask, binary=True):
    h, w, n = mask.shape[0], mask.shape[1], mask.shape[2]
    cdef RLEs Rs = RLEs(n)
    rleEncode(Rs._R,<byte*>mask.data,h,w,n)
    objs = _toString(Rs, int(binary))
    return objs
